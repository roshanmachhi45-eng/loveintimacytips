...
export default function ResultsDisplay({ result, onReset }: ResultsDisplayProps) {
  ...
  const handleWhatsAppShare = () => {
    const shareText = `Check out Loveons - Get personalized relationship tips and activities! Score: ${result.score}/100`;
  ...
  return (
    <div className="space-y-4 fade-in">
      ...
      <div className="result-card bg-white rounded-3xl shadow-xl shadow-rose-100 border border-rose-100 p-6">
        <div className="flex items-center gap-2 mb-4">
          <Sparkles className="w-5 h-5 text-purple-500" />
          <h3 className="font-display text-lg font-bold text-gray-800">Recommended Couple Activities</h3>
        </div>
        <p className="text-xs text-gray-400 mb-4">Based on your personalities and connection</p>
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="font-display font-bold text-gray-800 text-base">{act.name}</h4>
                </div>
                <span className={`text-xs font-semibold px-2 py-1 rounded-full ${difficultyColors[act.difficulty]}`}>
                  {act.difficulty}
                </span>
              </div>
              <p className="text-sm text-gray-600 leading-relaxed mb-2">{act.description}</p>
              <p className="text-xs text-rose-500 font-medium">
                <span className="font-semibold">Benefits:</span> {act.benefits}
              </p>
            </div>
          ))}
        </div>
